...
1. プログラミングの容易さ 
 ...
 - 動作を予測しやすいマルチタスク：すべてのメッセージング処理は協調的マルチタスキングによって実行されるため、並列で実行される他の処理が現在の処理に割り込んでくるタイミングが決定しており、あるコードの処理順を予測することが通常のマルチスレッドプログラムよりも圧倒的に簡単です。mutexやsemaphoreといったOSスレッドレベルの同期プリミティブなどを管理する必要もありません。
 ...
 - 多数の計算ノードを連携させる必要があるような、分散コンピューティング環境上でOrleansからinspireされた、celluloidよりも優れたセマンティクスを提供しています(後ほど出てくる仮想actorをご覧ください）
